    ...
    <div class="container px-6 mx-auto">
        <div class="flex justify-end">
            <a class="mb-4 px-5 my-6 py-3 font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"
               href="/vehicles/create" role="button">
                ...
                <table class="w-full whitespace-no-wrap">
                    ...
                    <form method="GET" action="{{ route('vehicles.index') }}">
                        <tr class="dark:bg-gray-800 text-gray-700 dark:text-gray-400 border-hidden">
                            <td class="px-4 py-3">
                                <input type="text" name="make" class="form-input block w-full dark:bg-gray-800"
                                       placeholder="Markė ar modelis" value="{{ request('make') }}">
                            </td>
                            <td class="px-4 py-3">
                                <input type="text" name="number_plate" class="form-input block w-full dark:bg-gray-800"
                                       placeholder="Valst. numeriai" value="{{ request('number_plate') }}">
                            </td>
                            <td class="px-4 py-3">
                                <input type="text" name="client_name" class="form-input block w-full dark:bg-gray-800"
                                       placeholder="Savininkas" value="{{ request('client_name') }}">
                            </td>
                            <td class="px-4 py-3">
                                <input type="text" name="vin" class="form-input block w-full dark:bg-gray-800"
                                       placeholder="VIN kodas" value="{{ request('vin') }}">
                            </td>
                            <td class="px-4 py-3">
                    ...
                    @unless(count($vehicles) == 0)
                        @foreach ($vehicles as $vehicle)
                            ...
                            <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <div class="flex items-center text-sm">
                                        <div class="relative hidden w-12 h-auto mr-3 rounded-full md:block">
                                            ...
                                            @if(file_exists(public_path($brandLogoPath)))
                                                <img class="w-12 h-auto " src="{{ asset($brandLogoPath) }}"
                                                     alt="{{ $vehicle->brand }}" loading="lazy"/>
                                            @else
                                                <img class="w-12 h-auto "
                                                     src="{{ asset('storage/assets/makes/default.png') }}" alt="Default"
                                                     loading="lazy"/>
                                            @endif
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center space-x-4 text-sm">
                                        ...
                                        <button
                                            class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray delete-button-vehicle"
                                            data-vehicle-id="{{ $vehicle->id }}" aria-label="Delete">
                                            <svg class="w-5 h-5" aria-hidden="true" fill="currentColor"
                                                 viewBox="0 0 20 20">
                            ...
                            <div id="confirmation-modal-{{ $vehicle->id }}"
                                ...
                                <div
                                    ...
                                    data-modal-id="confirmation-modal-{{ $vehicle->id }}">
                                    <button id="modal-close-button-{{ $vehicle->id }}"
                                    ...
                                    <p class="text-gray-600 mb-6 text-center dark:text-gray-200">Ar tikrai norite
                                        pašalinti automobilį <strong
                                            style="background-image: linear-gradient(to right, #f83d5c, #fd4b2f); color: transparent; -webkit-background-clip: text; background-clip: text;">{{ $vehicle->license_plate }}</strong>?
                                    </p>
                                    <div class="mt-6 flex justify-end">
                                        <button id="cancel-button-{{ $vehicle->id }}"
                                        ...
                                        <form id="delete-form-{{ $vehicle->id }}" method="POST"
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button id="confirm-delete-button-{{ $vehicle->id }}"
                        ...
                        @endforeach
                    @else
                        <h4 class="text-sm font-semibold text-gray-600 dark:text-gray-300">Nerasta automobilių
                            įrašų.</h4>
                    @endunless
                </table>
